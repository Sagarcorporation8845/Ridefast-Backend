...
const dbService = createDatabaseService('user-service');
monitor.registerService('user-service', dbService);

const connectDb = async () => {
  try {
    const connected = await dbService.connect();
    if (!connected) {
      console.error('❌ FATAL: [user-service] Failed to connect to the database.');
      process.exit(1);
    }
  } catch (err) {
    console.error('❌ FATAL: [user-service] Failed to connect to the database.');
